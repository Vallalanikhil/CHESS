...
function changedirection(num, direction) {
  ...
  switch (direction) {
    case "E":
      temp = queen.position.y + num; //N
      if (temp > 7 || temp < 0) {
        console.log("outside boundary E");
...
function play(input) {
  var value = input.split(" "); //split() method is used to split a string into an array of substrings, and returns the new array//
  value.forEach((Element) => {
    //forEach() method calls a function once for each element in an array, in order

    let num = Number(Element.slice(Element.length - 1)); //slice() method selects the elements starting at the given start argument, and ends at, but does not include, the given end argument.
    let direction = Element.slice(0, Element.length - 1);
    //console.log("Number =>" + num)
    //console.log("direction =>" + direction)

    changedirection(num, direction);
  });
